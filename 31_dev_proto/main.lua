...
-- Optional localhost visual-debug bridge (disabled unless explicitly enabled)
local debug_bridge = nil
if os.getenv("LOVE2D_DEBUG_BRIDGE") == "1" then
    local success, result = pcall(require, "engine.core.debug.bridge")
    if success then
        debug_bridge = result
    else
        print("[DebugBridge] Could not load: " .. tostring(result))
    end
end
...
function love.load()
    ...
    if debug_bridge then
        local success, err = debug_bridge:start()
        if not success then
            print("[DebugBridge] Could not start: " .. tostring(err))
            debug_bridge = nil
        end
    end
end
...
function love.update(dt)
    if debug_bridge then debug_bridge:update(dt) end
    local simulation_dt =
        debug_bridge and debug_bridge:getSimulationDelta(dt) or dt
    if simulation_dt then lifecycle:update(simulation_dt) end
end
...
function love.focus(focused)
    if focused then
        ...
        if sound:shouldAutoResumeBGM() and
           not sound.current_bgm:isPlaying() then
