...
function sound:_loadBGM(name, config)
    ...
    if info then
        ...
        local should_loop = config.loop
        if should_loop == nil then should_loop = true end
        self.bgm[name]:setLooping(should_loop)
...
function sound:pauseBGM()
    if self.current_bgm then
        ...
        if self.current_bgm:isPlaying() then
...
function sound:shouldAutoResumeBGM()
    return self.current_bgm ~= nil and
           not self.settings.muted and
           not self.bgm_paused
end
...
function sound:toggleMute()
    ...
    if self.settings.muted then
        ...
        if self.current_bgm and not self.bgm_paused then
            self.current_bgm:play()
        end
    end
