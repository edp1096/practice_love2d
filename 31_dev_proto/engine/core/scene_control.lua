...
function scene_control.switch(scene_or_name, ...)
    ...
    local old_scene = scene_control.current

    -- Call exit callback on current scene
    if old_scene and old_scene.exit then
        old_scene:exit()
    end

    -- A switch starts a new navigation branch. Keep one fallback for
    -- switch-then-pop screens, but discard any older pushed branch.
    scene_control.stack = {}
    scene_control.previous = old_scene
...
function scene_control.push(scene_or_name, ...)
    ...
    if old_scene and old_scene.pause then
        old_scene:pause()
    end
    if old_scene then
        table.insert(scene_control.stack, old_scene)
    end
...
function scene_control.pop()
    local target
    if #scene_control.stack > 0 then
        target = table.remove(scene_control.stack)
    else
        target = scene_control.previous
    end
    ...
    scene_control.current = target
    scene_control.previous = scene_control.stack[#scene_control.stack]
