-- Minimal deterministic JSON support for the localhost debug protocol.
--
-- The protocol accepts flat primitive parameters. Keeping request parsing small
-- avoids exposing a general Lua evaluator while responses can contain nested
-- tables through encode().
...
local function encode(value, seen)
    ...
    for _, entry in ipairs(entries) do
        parts[#parts + 1] =
            '"' .. escape(entry.key) .. '":' .. encode(entry.value, seen)
    end
...
function json.getPrimitive(line, name)
    local string_value = json.getString(line, name)
    if string_value ~= nil then return string_value end

    local boolean_value = json.getBoolean(line, name)
    if boolean_value ~= nil then return boolean_value end

    return json.getNumber(line, name)
end
