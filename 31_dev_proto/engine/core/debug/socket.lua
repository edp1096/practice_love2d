...
local function last_error()
    if is_windows then
        return tonumber(backend.WSAGetLastError())
    end
    return ffi.errno()
end

local function would_block(error_code)
    if is_windows then
...
local function is_invalid(handle)
    if handle == nil then return true end
    if is_windows then
        return handle == ffi.cast("SOCKET", -1)
    end
    return handle < 0
end

local function set_nonblocking(handle)
    if is_windows then
    ...
    local new_flags = ffi.new("int", bit.bor(flags, O_NONBLOCK))
    return backend.fcntl(handle, F_SETFL, new_flags) == 0
end

local function set_socket_options(handle)
...
function socket.startServer(port)
    ...
    if is_invalid(handle) then
        local error_code = last_error()
        socket.cleanup()
        return nil, "socket() failed: " .. error_code
    end
    set_socket_options(handle)
    if not set_nonblocking(handle) then
        socket.close(handle)
        socket.cleanup()
...
function socket.accept(server)
    ...
    if is_invalid(client) then
        local error_code = last_error()
        if would_block(error_code) then return nil end
        return nil, "accept() failed: " .. error_code
    end
    set_socket_options(client)
    if not set_nonblocking(client) then
        socket.close(client)
...
function socket.receive(client, maximum)
    ...
    local error_code = last_error()
    if would_block(error_code) then return nil, "wait" end
...
function socket.close(handle)
    if is_invalid(handle) then return end
    if is_windows then
