...
function persistence:saveCheckpoint(scene)
    ...
    self.checkpoint = {
        ...
        killed_enemies = utils:DeepCopy(entity_registry.killed_enemies),
        picked_items = utils:DeepCopy(entity_registry.picked_items),
        transformed_npcs = utils:DeepCopy(entity_registry.transformed_npcs),
        destroyed_props = utils:DeepCopy(entity_registry.destroyed_props),
    ...
    for name, system in pairs(self.registered_systems) do
        if system.save then
            self.checkpoint.systems_data[name] =
                utils:DeepCopy(system.save(scene))
        end
    end
end
...
function persistence:getCheckpoint()
    return utils:DeepCopy(self.checkpoint)
end
