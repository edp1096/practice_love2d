...
function system.preventDuplicateInstance(is_mobile)
    -- Test/debug processes opt out explicitly so automation never leaves or
    -- contends with the user's normal single-instance semaphore.
    if os.getenv("LOVE2D_DISABLE_INSTANCE_LOCK") == "1" or
       is_mobile or _VERSION ~= "Lua 5.1" then
        return
    end
