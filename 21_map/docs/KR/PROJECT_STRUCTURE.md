...
├── effects.lua           - 파티클 효과 시스템
├── hud.lua               - HUD 렌더링 (체력바, 쿨다운)
├── minimap.lua           - 미니맵 렌더링 시스템
...
│   ├── input_coordinator.lua           - 다중 입력 소스 조정
...
**목적:** 게임 전용 콘텐츠 (RPG Maker의 "프로젝트" 폴더와 유사).
...
lib/
├── screen/
│   └── init.lua          - 가상 해상도 시스템 (960x540)
├── text/
│   └── init.lua          - 텍스트 렌더링 유틸리티
└── ini/
    └── init.lua          - INI 파일 파서
```
